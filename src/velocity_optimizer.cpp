...
VelocityOptimizer::OptimizationResult VelocityOptimizer::optimize(const OptimizationData & data)
{
    ...
    std::vector<double> time_vec(N, 0.0);
    for(size_t i=1; i<N; ++i) {
        const double t = time_vec.at(i-1) + dt;
        time_vec.at(i) = t;
    }
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        // std::cout << "opt_vel[" << i << "]: " << opt_vel.at(i) << std::endl;
        std::cout << "opt_acc[" << i << "]: " << opt_acc.at(i) << "  opt_jerk[" << i << "]: " << opt_jerk.at(i) << std::endl;
    }
